...
class TestFilms:
    ...
    def test_create_film_from_db(self):
        ...
        resp = clinet.post('/films', data=json.dumps(data), content_type='application/json')
    ...
    def test_update_film_from_db(self):
        client = app.test_client()
    ...
    def test_update_film_mock_db(self):
        with patch('src.db.session.add', autospec=True) as mock_session_add, \
                patch('src.db.session.commit', autospec=True) as mock_session_commit, \
                patch('src.services.film_service.FilmService.fetch_film_by_uuid') as mock_query:
